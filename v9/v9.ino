...
byte currentWordIndex = 0;

// Track skipped words (by index into currentWords[])
byte skippedWordIndices[20];
byte totalSkippedWords = 0;
byte skippedWordsPage = 0;
  ...
  if (digitalRead(buttonSelect) == LOW && millis() - lastButtonPress > debounceDelay) {
    ...
    if (loadTheme(currentThemeIndex)) {
      if (totalWordsInTheme >= 20) {
        ...
        usedWords = 0;
        totalSkippedWords = 0;   // NEW: clear skipped words
        currentPlayer = 0;
        gameState = 2;
        displayNextWord();
      } else {
...
void displayNextWord() {
  ...
  currentWordIndex = wordIndex;   // remember which word is on screen
  // Display on LCD
  lcd.setCursor(0, 0);
  lcd.print(F("PLAYER "));
...
void checkSkipButton() {
  if (digitalRead(buttonSkip) == LOW && millis() - lastButtonPress > debounceDelay) {
    ...
    // remember this word as skipped
    if (totalSkippedWords < 20) {
      skippedWordIndices[totalSkippedWords] = currentWordIndex;
      totalSkippedWords++;
      Serial.print(F("Skipped word: "));
      Serial.println(currentWords[currentWordIndex]);
    }
...
void displayScoresScreen() {
  ...
  // We will always allow "SKP:Skipped" hint, even if list is empty.
  if (selectedPlayers <= 2) {
    lcd.print(F("SEL:Menu SKP:Skipd"));
  } else if (selectedPlayers == 3) {
    lcd.print(F("SEL:Menu SKP:Skipd"));
  } else if (selectedPlayers == 4) {
    if (scoresScreenPage == 0) {
      lcd.print(F("<->:More SEL,SKIP"));
    } else {
      lcd.print(F("<->:Back SEL,SKIP"));
    }
  }
}
...
void handleScoresScreen() {
  ...
  if (digitalRead(buttonSelect) == LOW && millis() - lastButtonPress > debounceDelay) {
    ...
    //open skipped words list with SKIP button, if any
  if (digitalRead(buttonSkip) == LOW && millis() - lastButtonPress > debounceDelay) {
    lastButtonPress = millis();
    if (totalSkippedWords > 0) {
      skippedWordsPage = 0;
      displaySkippedWordsScreen();   // sets gameState = 5
    } else {
      // Optional: show "NO SKIPPED WORDS" for a moment
      lcd.clear();
      lcd.setCursor(0, 1);
      lcd.print(F("NO SKIPPED WORDS"));
      delay(800);
      displayScoresScreen();
    }
    while (digitalRead(buttonSkip) == LOW) delay(10);
...
// ==================== SKIPPED WORDS SCREEN (STATE 5) ====================
void displaySkippedWordsScreen() {
  gameState = 5;
  lcd.clear();

  byte totalPages = (totalSkippedWords + 2) / 3;  // 3 words per page

  lcd.setCursor(0, 0);
  lcd.print(F("SKIPPED WORDS"));
  if (totalPages > 1) {
    lcd.print(F(" "));
    lcd.print(skippedWordsPage + 1);
    lcd.print(F("/"));
    lcd.print(totalPages);
  }

  // Show up to 3 words on lines 1-3
  for (byte line = 0; line < 3; line++) {
    lcd.setCursor(0, 1 + line);
    lcd.print(F("                    ")); // clear line
    
    byte idx = skippedWordsPage * 3 + line;
    if (idx < totalSkippedWords) {
      lcd.setCursor(0, 1 + line);
      lcd.print(F("- "));
      lcd.print(currentWords[skippedWordIndices[idx]]);
    }
  }
}

void handleSkippedWordsScreen() {
  byte totalPages = (totalSkippedWords + 2) / 3;
  if (totalPages == 0) {
    // Safety: nothing to show, go back to scores
    gameState = 4;
    displayScoresScreen();
    return;
  }

  // Navigate pages with left/right
  if (digitalRead(buttonLeft) == LOW && millis() - lastButtonPress > debounceDelay) {
    lastButtonPress = millis();
    if (totalPages > 1) {
      skippedWordsPage = (skippedWordsPage + totalPages - 1) % totalPages;
      displaySkippedWordsScreen();
    }
    while (digitalRead(buttonLeft) == LOW) delay(10);
  }
  ...
  if (digitalRead(buttonRight) == LOW && millis() - lastButtonPress > debounceDelay) {
    lastButtonPress = millis();
    if (totalPages > 1) {
      skippedWordsPage = (skippedWordsPage + 1) % totalPages;
      displaySkippedWordsScreen();
    }
    while (digitalRead(buttonRight) == LOW) delay(10);
  }

  // Select = back to scores screen
  if (digitalRead(buttonSelect) == LOW && millis() - lastButtonPress > debounceDelay) {
    lastButtonPress = millis();
    gameState = 4;
    displayScoresScreen();
